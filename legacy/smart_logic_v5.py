...
def parse_md_to_sections(md: str) -> dict[str, dict]:
    ...
    Only returns sections that have non-empty body content. Standalone
    bullet points without indented children are treated as preamble and
    are not included as sections.
    """
    sections = {}
    ...
    # Filter out sections with no body (standalone statements)
    # or sections where body only contains empty/whitespace lines
    return {
        k: v for k, v in sections.items()
        if v["body"] and any(line.strip() for line in v["body"])
    }
